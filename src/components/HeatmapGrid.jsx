 ...
 */
export default function HeatmapGrid({ heatmapData, theme }) {
      ...
      <div className="relative flex gap-[3px] overflow-hidden py-1" aria-label="Contribution heatmap">
        {heatmapData.map((week, wi) => (
          <div key={wi} className="flex flex-col gap-[3px]">
            {week.map((cell, di) => {
              if (!cell) return null; // safety check
              return (
                <motion.div
                  key={di}
                  initial={{ opacity: 0 }}
                  ...
                  className={`w-[11px] h-[11px] rounded-[2px] transition-transform duration-200 hover:scale-125 hover:z-10 relative cursor-crosshair
                ...
                >
                  {/* Tooltip */}
                  {hoveredCell === cell && (
                    <div className="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-2 py-1 bg-gray-900 border border-white/10 rounded text-[10px] whitespace-nowrap text-white/90 z-20 pointer-events-none shadow-xl flex flex-col items-center">
                      <span className="font-medium text-white">{cell.count} {cell.count === 1 ? "contribution" : "contributions"}</span>
